...
--  The specification and implementation relies on a resolution for
--  System.Duration being at least one millisecond = 0.001.  The AVR
--  GNAT compiler is patched to provide a 32-bit wide type for
--  duration that has a tick of at least one millisecond. Duration
--  also spans at least 2 days so that day_duration'last +
--  day_duration'last does not hit the upper limit of the base type
--  duration.
--
...
--        end;
   ...
   function "+" (Left : Time; Right : Duration) return Time is
      ...
      Result  : Time;
   begin
      Result.Days := Left.Days;
      Seconds := Left.Secs + Right;
      while Seconds > Day_Duration'Last loop
         Seconds := Seconds - Day_Duration'Last;
         Result.Days := Result.Days + 1;
      end loop;
      while Seconds < 0.0 loop
         Seconds := Seconds + Day_Duration'Last;
         Result.Days := Result.Days - 1;
      end loop;
   ...
   begin
      ...
      if Left.Days = Right.Days then
         return Duration (Left.Secs - Right.Secs);
      else
         declare
            Day_Diff : constant Day_Count := Left.Days - Right.Days;
         begin
            if Day_Diff >= 2 then
               return Duration'Last;
            else
               -- Day_Diff = 1
               return Duration (86_400.0 + Left.Secs - Right.Secs);
            end if;
         end;
      end if;
   end "-";
   ...
   begin
      if Left.Days > Right.Days then
         return True;
      elsif Left.Days = Right.Days and then Left.Secs > Right.Secs then
         return True;
      else
         return False;
      end if;
   ...
   function Time_Image_Short (T : Time) return AStr6 is
      use AVR.Int_Img;
      Image       : AStr6;
      H           : constant Hour_Number   := Hour (T);
      M           : constant Minute_Number := Minute (T);
      S           : constant Second_Number := Second (T);
   begin
      U8_Img_99_Right (H, Image(1..2));
      U8_Img_99_Right (M, Image(3..4));
      U8_Img_99_Right (S, Image(5..6));
      return Image;
   ...
   is
      ...
      Days        : Natural    := 0;
      Hours       : Hour_Number   := 0;
      Mins        : Minute_Number := 0;
      Secs        : Second_Number := 0;
   begin
      if D < 0.0 then
         Is_Negative := True;
         D := -D;
      ...
      Img(7..12) := Millisec_Image (D)(5..10);
      if Secs < 10 then Img(7) := '0'; end if;
      Img(6) := ':';
      if Mins < 10 then Img(4) := '0'; end if;
      Img(3) := ':';
      if Hours < 10 then Img(1) := '0'; end if;
      return Img;
   end Fract_Image;
